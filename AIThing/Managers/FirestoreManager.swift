    ...
    func getApiKeyAnthropic() async -> String {
        do {
            ...
            AnalyticsManager.shared.customEvent(
                view: .FirebaseManager,
                primary: .firebase,
                secondary: "get_anthropic_api_key",
                sev: .info
            )
        ...
        do {
            ...
            if let agent = try? snapshot.data(as: ManagedGitHubAgent.self) {
                AnalyticsManager.shared.customEvent(
                    view: .FirebaseManager,
                    primary: .firebase,
                    secondary: "get_managed_github_agent",
                    sev: .info
                )
                return agent
            }
    ...
    func getManagedAgents() async -> [McpServer] {
        ...
        do {
            ...
            for doc in snapshot.documents {
                if let server = try? doc.data(as: McpServer.self) {
                    let s = McpServer(
                        id: doc.documentID,
                        image: server.image,
                        name: server.name,
...
extension FirestoreManager {
        ...
        for order in orders {
            if let credits = planCredits[order.planId] {
                ...
                AnalyticsManager.shared.customEvent(
                    view: .FirebaseManager,
                    primary: .firebase,
                    secondary: "fetch_credits_missing_plan_details",
                    sev: .error
                )
    ...
    func getPlanDetailsMap() async -> [String: Int] {
        ...
        do {
            ...
            AnalyticsManager.shared.customEvent(
                view: .FirebaseManager,
                primary: .firebase,
                secondary: "fetch_credits_error_plan_details",
                sev: .error
            )
            logger.error(
                    ...
                    do {
                        ...
                        do {
                            ...
                            AnalyticsManager.shared.customEvent(
                                view: .FirebaseManager,
                                primary: .firebase,
                                secondary: "fetch_credits_error_reading_orders",
                                sev: .error
                            )
                            logger.error(
        ...
        -> PlanOrder?
    {
        ...
        AnalyticsManager.shared.customEvent(
            view: .FirebaseManager,
            primary: .firebase,
            secondary: "fetch_credits_error_decoding_date",
            sev: .error
        )
        return nil
    }
